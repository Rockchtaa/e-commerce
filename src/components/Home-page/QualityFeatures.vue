<template>
  <div class="quality-features py-10">
    <v-container>
      <h2 class="text-center mb-6">Quality Features</h2>
      <v-row justify="center">
        <v-col cols="12" sm="6" md="4" lg="2" v-for="item in features" :key="item.title">
          <v-card class="text-center d-flex flex-column align-center justify-space-between pa-4 same-height-card" elevation="2">
            <div v-html="item.svg" class="feature-icon"></div>
            <v-card-title class="font-weight-bold mt-3 flex-grow-1">{{ item.title }}</v-card-title>
            <v-card-text class="text-muted flex-grow-1">{{ item.description }}</v-card-text>
          </v-card>
        </v-col>
      </v-row>
    </v-container>
  ...
  data: () => ({
    features: [
      ...
      },
      {
        ...
        description: "We provide 24/7 customer support to our customers.",
