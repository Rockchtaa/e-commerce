<template>
  <div class="new-products pt-12">
    <v-container fluid>
      ...
      <v-row>
        <v-col cols="7" style="margin-top: 100px;">
            ...
            <swiper-slide v-for="product in products" :key="product.id">
              <div class="card-wrapper">
                <v-card elevation="0" class="pa-5" style="position: relative">
                  ...
                  <v-btn density="compact" width="80" variant="outlined" height="30" class="bg-white quick-view" style="
                text-transform: none;
                position: absolute;
                top: 35%;
                left: 50%;
              ...
              " @click="openQuickView(product)">
                  ...
                  <v-rating :model-value="product.rating" half-increments readonly color="yellow-darken-2"
                ...
                border-radius: 30px;
                min-width: 150px;
              " variant="outlined" @click="navigateToProductDetails(product.id)">
                      Choose Options
                    </v-btn>
                  </div>
                </v-card>
          ...
          <v-dialog v-model="quickViewOpen" max-width="600px">
            <v-card>
              ...
              <v-card-text>
                <img :src="selectedImages[selectedProduct?.id] || selectedProduct?.thumbnail
                  " class="w-100 mb-4" style="height: 300px; object-fit: cover" />
                <div>{{ selectedProduct?.description }}</div>
                <div class="mt-2">
                  <v-rating :model-value="selectedProduct?.rating" half-increments readonly color="yellow-darken-2"
                    size="small" />
                </div>
                <div class="mt-2">
              ...
              <v-card-actions>
                ...
                <v-btn color="primary" @click="navigateToProductDetails(selectedProduct?.id)">
                  View Details
                </v-btn>
              </v-card-actions>
            </v-card>
